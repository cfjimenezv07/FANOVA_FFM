# Intervals TWA+OWA+FFM sequential
...
FM_FFM_fun_int_eval_sequential <- function(raw_data, smooth_data, fh, fore_method, alpha,Fixed_TWA)
{
    ...
    for(iwk in 1:(11 - fh))
    {
        ...
        {
            fore_validation[,,(ij - 1)] = FM_FFM_fun(data_set_array = (smooth_data[,,1:ij]), fh = fh, fmethod = fore_method,Fixed_TWA)
            rm(ij)
        }
        ...
        point_forecast <- exp(FM_FFM_fun(data_set_array = (smooth_data[,,1:(n_validation + iwk)]), fh = fh, fmethod = fore_method,Fixed_TWA))
        for(iw in 1:n_state)
        {
...
load("~/Library/CloudStorage/GoogleDrive-cristian.jimenezvaron@york.ac.uk/My Drive/FANOVA_FFM/Rcodes/Load_TWA_OWA_FFM.RData")
...
for(iw in 1:10)
{
    sequential_int_score_list_female_arima[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_female_3d, 
                                                                             smooth_data = TWA_res_female, 
                                                                             fh = iw, fore_method = "arima", alpha = 0.2,Fixed_TWA=TWA_Fixed_female)
 
    sequential_int_score_list_male_arima[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_male_3d, 
...
for(iw in 1:10)
{
    sequential_int_score_list_female_ets[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_female_3d, 
                                                                                 smooth_data = TWA_res_female, 
                                                                                 fh = iw, fore_method = "ets", alpha = 0.2,Fixed_TWA=TWA_Fixed_female)
    
    sequential_int_score_list_male_ets[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_male_3d, 
                                                                               smooth_data = TWA_res_male, 
...
for(iw in 1:10)
{
    sequential_int_score_list_female_arima_PI_95[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_female_3d, 
                                                                                   smooth_data = TWA_res_female, 
                                                                                   fh = iw, fore_method = "arima", alpha = 0.05,Fixed_TWA=TWA_Fixed_female)

    sequential_int_score_list_male_arima_PI_95[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_male_3d, 
                                                                                 smooth_data = TWA_res_male, 
...
for(iw in 1:10)
{
    sequential_int_score_list_female_ets_PI_95[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_female_3d, 
                                                                                 smooth_data = TWA_res_female, 
                                                                                 fh = iw, fore_method = "ets", alpha = 0.05,Fixed_TWA=TWA_Fixed_female)
    
    sequential_int_score_list_male_ets_PI_95[[iw]] = FM_FFM_fun_int_eval_sequential(raw_data = truth_male_3d, 
                                                                               smooth_data = TWA_res_male, 
