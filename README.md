...
#### 2. Functional ANOVA (FANOVA) Models
* **One_way_FANOVA.R**: Performs the baseline One-Way Functional ANOVA.
* **FANOVA_MFTS_Arima.R**: Functional ANOVA with Multivariate Functional Time Series using ARIMA.
* **FANOVA_MFTS_ets.R**: Functional ANOVA with Multivariate Functional Time Series using Exponential Smoothing (ETS).
...
#### 3. Functional Factor Models (FFM)
* **FFM.R**: Functional Factor Model implementation.

#### 4. Estimation Procedures for point and interval forecast
* **TWA-FFM.R**: Two-Way Analysis (TWA) integrated with FFM.
* **TWA-OWA-FFM.R**: Hybrid Two-Way and One-Way Analysis plus FFM framework.
